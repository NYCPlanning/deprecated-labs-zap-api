const postProcess = {
  ...
  bbl: postProcessBbls,
  action: postProcessActions,
  milestone: postProcessMilestones,
  keyword: postProcessKeywords,
  applicant:  postProcessApplicantTeam,
...
function postProcessBbls(bbls) {
  return bbls.map(bbl => bbl.dcp_bblnumber);
}
...
function postProcessActions(actions) {
  ...
  return actions.filter(action => {
    ...
    if(actioncodeMatch) {
      action.actioncode = actioncodeMatch[1];
      action.dcp_name = actioncodeMatch[2];
      return true;
    }
...
function postProcessMilestones(milestones, publicStatus, ulurpNonUlurp) {
...
function postProcessKeywords(keywords) {
  return keywords.map(keyword => keyword._dcp_keyword_value_formatted);
}

function postProcessApplicantTeam(applicantTeam) {
  return applicantTeam.map(team => {
    team.role = team.dcp_applicantrole_formatted;
    team.name = team._dcp_applicant_customer_value_formatted;
    return team;
  });
}
