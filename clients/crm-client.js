...
class CRMClient {
  ...
  async doFetch(method, query, data = null, isBatch = false) {
    try {
      ...
      if (isBatch && res.status === 200) {
        text = CRMClient.extractBatchData(text);
      }

      try {
        const json = JSON.parse(text, this.dateReviver);
        // some errors are returned as Message property on top-level json object
        if (json.Message && json.ErrorCode) json.error = { message: json.Message };
